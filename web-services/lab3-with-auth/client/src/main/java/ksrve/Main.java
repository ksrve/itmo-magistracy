...
//        System.setProperty("com.sun.xml.internal.ws.transport.http.HttpAdapter.dumpTreshold", "999999");
        ...
        try {
            ...
            URL endpoint = new URL(String.format("http://%s:%s/EmployeeService?wsdl", jArgs.getHost(), jArgs.getPort()));
            EmployeeService service = new EmployeeService(endpoint);
            Authorizer authorizer = new Authorizer(endpoint, service);

            var port = authorizer.getAuthenticatedServicePort(jArgs.getUsername(), jArgs.getPassword());
            commandExecutor.run(port);
